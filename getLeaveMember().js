...
function getLeaveMember(){
    try {
        DBCopy();
        var cur = DB.rawQuery("select chat_id,user_id,message,v,attachment from chat_logs where v LIKE '%DELMEM%' ORDER BY id DESC LIMIT 1", null);
        ...
        var nickname = c[0].message.member.nickName;
